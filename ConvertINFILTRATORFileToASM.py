...
# E.g.
# python3 ConvertINFILTRATORFileToASM.py --model=C64 --input=Filename --output=Filename
...
#-------------------------------------------------------------------
# Added By OSK
strNewFile = f".label SCREENRAM = ${right(hex(SCREENRAM), len(hex(SCREENRAM))-2).zfill(4).upper()}" + "\n"
strNewFile += f".label COLOURRAM = ${right(hex(COLOURRAM), len(hex(COLOURRAM))-2).zfill(4).upper()}" + "\n"
strNewLine = ""
...
for tmp in file:
    ...
    tmp = tmp.split(",")
    IOMap.append(tmp)
    strNewFile += f".label {tmp[1]} = {tmp[0]}" + "\n"
